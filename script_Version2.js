...
function generateFlights(origin, destination, dateStr, filter = null) {
    const flights = [];
...
// ============ PARSE COMMANDS - FIXED ============

function parseANCommand(input) {
    // Comprehensive regex to handle all AN formats:
    // AN10JUNDACBKK
    // AN10JUNDACBKK/ABG
    // AN15JUL26DACSIN
    // AN15JUL26DACSIN/AEK
    ...
    // ^AN - starts with AN
    // (\d{2}) - day (2 digits)
    // ([A-Z]{3}) - month (3 letters)
    // (\d{0,4}) - year (0-4 digits, optional)
    // ([A-Z]{3}) - origin airport
    // ([A-Z]{3}) - destination airport
    // (?:\/([A-Z0-9]{2}))? - optional airline filter /XX
    // $ - end of string
    
    const pattern = /^AN(\d{2})([A-Z]{3})(\d{0,4})([A-Z]{3})([A-Z]{3})(?:\/([A-Z0-9]{2}))?$/;
    ...
    if (!yearStr || yearStr.length === 0) {
        ...
        fullYear = twoDigitYear > 50 ? 1900 + twoDigitYear : 2000 + twoDigitYear;
    } else if (yearStr.length === 4) {
        fullYear = parseInt(yearStr);
    } else {
        // 1 or 3 digits - assume current century
        fullYear = 2000 + parseInt(yearStr);
    }
...
function parseTTPCommand(input) {
    // TKTL20JUN or TKOK or TTTL20JUN
    const pattern = /^T[KT](.*)$/;
    const match = input.match(pattern);
    if (!match) return null;
...
function handleAN(parsed) {
    const { origin, destination, dateStr, airlineFilter, day, month, year } = parsed;
    
    // Validate airports
    if (!AIRPORTS[origin]) {
        addTerminalLine('INVALID ORIGIN AIRPORT', 'error');
        return;
    }
    
    if (!AIRPORTS[destination]) {
        addTerminalLine('INVALID DESTINATION AIRPORT', 'error');
        return;
    }
    ...
    addTerminalLine(`${day}${month}${year} ${AIRPORTS[origin].name} TO ${AIRPORTS[destination].name}`);
    addBlankLine();
    
    // Format: 1 BG147 Y9 B7 M5 H4 DAC BKK 1030 1445
    flights.forEach((flight, index) => {
        const classStr = BOOKING_CLASSES
            .filter(cls => flight.classes[cls])
            .map(cls => `${cls}${flight.classes[cls]}`)
            .join(' ');
        
        const line = `${index + 1} ${flight.airline}${flight.flightNum} ${classStr} ${origin} ${destination} ${flight.departure} ${flight.arrival}`;
...
function handleSS(parsed) {
    ...
    if (segmentNum > session.lastAvailability.length || segmentNum < 1) {
    ...
    if (!BOOKING_CLASSES.includes(bookingClass)) {
        addTerminalLine('INVALID BOOKING CLASS', 'error');
        return;
    }
...
function handleXE(parsed) {
    ...
    if (segmentNum > session.segments.length || segmentNum < 1) {
        addTerminalLine('INVALID SEGMENT', 'error');
        return;
    }
...
function processCommand(input) {
    ...
    // Try to parse and execute - order matters for prefix matching
    let handled = false;
    
    // Check for FX commands first (they share FX prefix)
    if (cmd.startsWith('FXB')) {
