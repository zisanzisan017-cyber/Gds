...
function processCommand(cmd) {
    ...
    // FQD - Fare Quote Display (Before booking)
    if (cmd.startsWith('FQD')) {
        handleFQD(cmd);
        return;
    }

    // FPD - Payment Method (FPD CASH)
    if (cmd.startsWith('FPD')) {
        state.pnr.payment = cmd.substring(3).trim();
        printToTerminal(`OK - PAYMENT METHOD ${state.pnr.payment} ADDED`);
        return;
    }

    // XI - Delete Segment (XI1)
    ...
    // FQB Command
        ...
        if (state.pnr.flights.length > 0) {
            ...
            printToTerminal(`BEST FARE: USD ${fare} TAX INCLUDED (QUOTE ONLY)`);
        } else {
    ...
    // FXP Command
        ...
        if (state.pnr.flights.length > 0 && state.pnr.names.length > 0) {
            ...
            printToTerminal("OK - FARE STORED AND LOADED INTO PNR");
        } else {
            printToTerminal("ERROR: NAMES OR FLIGHTS MISSING", "error");
        }
    }
    // TTP Command
    else if (cmd === 'TTP') {
        if (state.pnr.isSaved && state.pnr.fareLoaded && state.pnr.payment) {
            state.pnr.isIssued = true;
            printToTerminal("OK - TICKET ISSUED SUCCESSFULLY");
        } else {
            printToTerminal("ERROR: PNR NOT SAVED, FARE NOT LOADED OR PAYMENT MISSING", "error");
        }
    }
    // TTR/P Command
    else if (cmd === 'TTR/P') {
...
`;
    ...
    downloadBtn.onclick = function() {
        ...
        doc.text("------------------------------------------", 10, 30);
        doc.text(`PASSENGER NAME: ${state.pnr.names[0]}`, 10, 40);
        ...
        doc.save(`Ticket_${state.pnr.names[0].replace('/', '_')}.pdf`);
    };
...
function showHelp() {
    ...
    printToTerminal("FQD[ORIGIN][DEST] - Fare Quote Display");
